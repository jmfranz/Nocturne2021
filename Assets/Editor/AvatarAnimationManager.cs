/*
    MotionCaptureEdior: A Unity editor extension window to handle captured motion and audio for use with Microsoft RocketBox Avatars.
    @Author: Matt P.
*/
using System;
...
public class AvatarAnimationManager : EditorWindow {
    ...
    public static void ShowWindow () {
        GetWindow<AvatarAnimationManager> ("Avatar Animation Manager");
    }
    ...
    void OnGUI () {
        ...
        if (avatarInstance == null) {
            ...
            if (GUILayout.Button ("Spawn Selected Items")) {
                ...
                //TODO: Consider a less hardcoded method for connecting the MJaw component and implement error checking. 
        ...
        if (avatarInstance != null) {
            if (GUILayout.Button ("Create PreFab object based on spawned items")) {

                //Set the path as within the Assets folder, and name it as the GameObject's name with the .prefab format
                string localPath = "Assets/" + avatarInstance.name + ".prefab";
                ...
                if (AssetDatabase.LoadAssetAtPath (localPath, typeof (GameObject))) {
                    ...
                    //If the user presses the yes button, create the Prefab
                    {
                        ...
                        UnityEngine.Object prefab = PrefabUtility.CreatePrefab (localPath, avatarInstance);
                        PrefabUtility.ReplacePrefab (avatarInstance, prefab, ReplacePrefabOptions.ConnectToPrefab);
                    }
                }
                //If the name doesn't exist, create the new Prefab
                else {
                    Debug.Log (avatarInstance.name + " is not a Prefab, will convert");
                    //Create a new Prefab at the path given
                    UnityEngine.Object prefab = PrefabUtility.CreatePrefab (localPath, avatarInstance);
                    PrefabUtility.ReplacePrefab (avatarInstance, prefab, ReplacePrefabOptions.ConnectToPrefab);
                }
